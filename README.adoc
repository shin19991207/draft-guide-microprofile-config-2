...
// The active Config Profile is specified as [hotspot=config-profile file=0]`maintaining` in the [hotspot file=0]`META-INF/microprofile-config.properties` property file, the default configuration source for an application that uses MicroProfile Config.
