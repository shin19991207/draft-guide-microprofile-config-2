...
Inject property [hotspot=email2 file=1]`io_openliberty_guides.email` as an [hotspot=email file=0]`Optional` value. Contrary to natively injecting the configured value, this avoid `NoSuchElementException` to be thrown when running the application in the default environment where the value is missing.
